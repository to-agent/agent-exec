...
async function runTests() {
	...
	chk('404 JSON default records fallback result', r.json?.result?.status, 404)
	chk('404 JSON default result reason is not_found', r.json?.result?.reason, 'not_found')
	chk('404 JSON default falls back to SJS document', r.json?.fallback?.url, '/SKILL.s.js')
	chk('404 JSON default fallback uses GET', r.json?.fallback?.method, 'GET')
	chk('404 JSON default candidate is path-local SKILL', r.json?.candidates?.[0]?.url, '/missing/SKILL.s.js')
	chk('404 JSON default candidate is unverified', r.json?.candidates?.[0]?.verified, false)
	chk('404 JSON default has no suggest list', Object.prototype.hasOwnProperty.call(r.json || {}, 'suggest'), false)
	chk('404 JSON default has no skill shortcut', Object.prototype.hasOwnProperty.call(r.json || {}, 'skill'), false)
	...
	chk('404 explicit markdown includes result section', r.text.includes('## Result'), true)
	chk('404 explicit markdown falls back to SJS document', r.text.includes('GET /SKILL.s.js'), true)
	chk('404 explicit markdown includes unverified candidate', r.text.includes('/missing/SKILL.s.js') && r.text.includes('verified: false'), true)
	chk('404 explicit markdown does not use hint prose', !r.text.includes('Read /SKILL.md first'), true)

	r = await request(`${BASE}/missing.json`)
	chk('GET /missing.json → 404', r.status, 404)
	chk('404 JSON uses SJS fallback', r.json?.fallback?.document, '/SKILL.s.js')
	chk('404 JSON strips format suffix for candidate', r.json?.candidates?.[0]?.url, '/missing/SKILL.s.js')
	chk('404 JSON includes alternate document refs', r.json?.refs?.includes('/SKILL.json'), true)
	chk('404 JSON does not expose direct exec suggestion', JSON.stringify(r.json).includes('POST /api/exec'), false)
	...
	chk('404 explicit HTML links SJS fallback document', r.text.includes('href="/SKILL.s.js"'), true)
	chk('404 explicit HTML includes unverified candidate', r.text.includes('/missing/SKILL.s.js') && r.text.includes('verified: false'), true)
	...
	chk('404 browser/html Accept links SJS fallback', r.text.includes('href="/SKILL.s.js"'), true)

	r = await request(`${BASE}/alpha/beta/gamma.json`)
	chk('GET /alpha/beta/gamma.json → 404', r.status, 404)
	chk('404 deep JSON candidate includes local SKILL', r.json?.candidates?.[0]?.url, '/alpha/beta/gamma/SKILL.s.js')
	chk('404 deep JSON candidate includes parent SKILL', r.json?.candidates?.[1]?.url, '/alpha/beta/SKILL.s.js')
	chk('404 deep JSON candidates have no method', Object.prototype.hasOwnProperty.call(r.json?.candidates?.[0] || {}, 'method'), false)
	...
	chk('404 .s.js includes unverified candidate', r.text.includes('m.candidates = [') && r.text.includes('verified: false'), true)
	chk('404 .s.js includes alternate refs', r.text.includes('m.refs = [') && r.text.includes('"/SKILL.json"'), true)
